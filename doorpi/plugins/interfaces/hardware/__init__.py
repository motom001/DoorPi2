...
class SetOutputAction(DOORPI.events.action_base_class): pass

class UnknownInputException(Exception): pass
class UnknownOutputException(Exception): pass
class UnknownLevelException(Exception): pass
class InputNameAlreadyExistsException(Exception): pass
class OutputNameAlreadyExistsException(Exception): pass
...
class HardwareInterfaceBaseClass(InterfaceBaseClass):
    ...
    def input_name_variations(self):
        ...
        for input_id in self.inputs.keys():
            for interface_name in self.interface_name_variations(postfix = '.'):
                ...
                input_variations["%s%s"%(interface_name, self.inputs[input_id].technical_name)] = self.inputs[input_id].id
    ...
    def output_name_variations(self):
        ...
        for output_id in self.outputs.keys():
            for interface_name in self.interface_name_variations(postfix = '.'):
                ...
                output_variations["%s%s"%(interface_name, self.outputs[output_id].technical_name)] = self.outputs[output_id].id
        return output_variations

    @property
    def channel_events(self, input_events = DOORPI.CONST.INTERFACES_HARDWARE_DEFAULT_INPUT_EVENTS, output_events = DOORPI.CONST.INTERFACES_HARDWARE_DEFAULT_OUTPUT_EVENTS):
        possible_events = []
        for input_name in self.inputs.keys():
            possible_events = possible_events + self._create_event_variations(self.inputs[input_name].technical_name, input_events)
            possible_events = possible_events + self._create_event_variations(self.inputs[input_name].name, input_events)

        for output_name in self.outputs.keys():
            possible_events = possible_events + self._create_event_variations(self.outputs[output_name].technical_name, output_events)
            possible_events = possible_events + self._create_event_variations(self.outputs[output_name].name, output_events)

        return possible_events

    def _create_event_variations(self, input, base_events):
        all_event_variations = []
        for event in base_events:
            for event_variations in [
                "%s"%event,                                                        # OnInputActive
                #"%s.%s"%(event, self.name),                                        # mein_GPIO_Board.OnInputActive
                "%s.%s"%(self.module_name, event),                        # plugins.interfaces.hardware.gpio.OnInputActive
                "%s.%s.%s"%(self.module_name, self.name, event),          # plugins.interfaces.hardware.gpio.mein_GPIO_Board.OnInputActive
                #"%s.%s"%(input, event),                                            # 16.OnInputActive
                #"%s.%s.%s"%(self.name, input, event),                              # mein_GPIO_Board.16.OnInputActive
                #"%s.%s.%s"%(self.__class__.__module__, input, event),              # plugins.interfaces.hardware.gpio.16.OnInputActive
                "%s.%s.%s.%s"%(self.module_name, self.name, input, event) # plugins.interfaces.hardware.gpio.mein_GPIO_Board.16.OnInputActive
    ...
    def _register_outputs_actions(self):
        for output_id in self.outputs.keys():
            ...
            DOORPI.events.register_action(
                high_level_object,
                *self._create_event_variations(self.outputs[output_id].name, DOORPI.CONST.INTERFACES_HARDWARE_INCOMING_OUTPUT_EVENTS_HIGH)
            )
            DOORPI.events.register_action(
                low_level_object,
                *self._create_event_variations(self.outputs[output_id].name, DOORPI.CONST.INTERFACES_HARDWARE_INCOMING_OUTPUT_EVENTS_LOW)
            )
    ...
    def get_input_id_by_technical_name(self, technical_name):
        for input_id in self.inputs:
            if self.inputs[input_id].technical_name == technical_name: return input_id
        return None

    def get_output_id_by_technical_name(self, technical_name):
        for output_id in self.outputs:
            if self.outputs[output_id].technical_name == technical_name: return output_id
        return None

class SingleChannel(object):
    actions = []
    def __init__(self, id, name, technical_name, initial_value = False, log = True, high_level = True, low_level = False, high_by_event = None, low_by_event = None):
        self.id = id
        self.name = name
